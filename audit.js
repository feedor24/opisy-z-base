...
(function (global) {
  ...
  function buildReport(a, ctx) {
    ...
    const trackUrl = ctx.trackUrl || 'https://opisy.mediafy.com.pl/api/track';
...
'<form class="form" id="lead-form"><h3>Napisz do mnie</h3><label>Firma</label><input name="firma" placeholder="Nazwa firmy"><label>Imię i nazwisko</label><input name="osoba"><label>E-mail *</label><input name="email" type="email" required placeholder="ty@firma.pl"><label>Telefon</label><input name="telefon" type="tel" placeholder="+48 …"><label>Wiadomość</label><textarea name="wiadomosc" placeholder="Np. Chcę poprawić opisy na ' + chName + ' dla całego katalogu."></textarea><input name="hp" style="display:none" tabindex="-1" autocomplete="off"><label class="cb"><input type="checkbox" name="zgoda" required style="width:auto;margin-top:3px"> Zgadzam się na kontakt Mediafy w sprawie moich ofert. Dane trafiają tylko do Mediafy (<a href="https://opisy.mediafy.com.pl/polityka-prywatnosci" target="_blank">polityka prywatności</a>).</label><button type="submit">Wyślij — odezwę się w 1 dzień roboczy</button><div class="err" id="lead-err"></div><div class="alt">Wolisz maila? <a href="mailto:maciej.fidor@mediafy.com.pl?subject=' + encodeURIComponent('Audyt ' + ctx.channelLabel + ' — ' + a.total + '/100') + '">Napisz bezpośrednio</a></div></form></div>' +
'<div class="foot">Raport wygenerowany przez darmowe narzędzie <a href="https://opisy.mediafy.com.pl">Opisy z BASE</a> (MAMS · Mediafy) na podstawie danych z API BASE. Limity długości tytułów są orientacyjne — sprawdź aktualne wymagania kanału. Progi jakości (300 zn., 800 zn., struktura) to praktyka z wdrożeń, nie oficjalne reguły marketplace\'ów. BASE i BaseLinker są znakami towarowymi ich właściciela.</div>' +
